...
class UserDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        uid, feature, course, subgroup = self.datas[idx]
        if self.do_mask:
...
class CourseClassifier(nn.Module):
    def __init__(self, in_feat=FEATURE_NUM+4, out_feat=COURSE_NUM+1, dropout=0.0):
        super().__init__()
...
class SubgroupClassifier(nn.Module):
    def __init__(self, in_feat=FEATURE_NUM+4, out_feat=SUBGROUP_NUM+1, dropout=0.0):
        super().__init__()
        self.in_feat = in_feat
        self.out_feat = out_feat
        self.non_idx = 0
