...
def parse_args():
    ...
    parser.add_argument("--course_batch_size", type=int, default=4)
    parser.add_argument("--gradient_accumulate", type=int, default=8)
    parser.add_argument("--extr_lr", type=float, default=5e-5)
    parser.add_argument("--lr", type=float, default=8e-5)
    parser.add_argument("--warmup_step", type=int, default=0)
    parser.add_argument("--metrics", type=str, default="mAP")
    parser.add_argument("--emb_dim", type=int, default=312)
...
def prepare_env(args):
    ...
    args.save_extr_model_file = os.path.join(args.dir,
                                             args.save_extr_model_file)
...
def predict_step(args,
                 courseset, userset,
                 extr, user_extr, course_extr,
                 loss_fn,
                 do_loss=True, do_mAP=True):
    extr.eval()
    user_extr.eval()
    course_extr.eval()
    with torch.no_grad():
        course_embed = course_extr(
            extr_course_embedding(extr,
                                  courseset,
                                  args.device)
        )
    loss = []
    ...
    for uid, user_desc, course_tar, _ in userset:
        with torch.no_grad():
            ...
            if do_loss:
                ...
                loss.append(loss_fn(predict, course_tar).item())
            if do_mAP:
                mAPk.append(
                    onehot2mAPk(predict,
                                course_tar,
    ...
    return uids, predicts, np.mean(loss), np.mean(mAPk)
...
def main(args):
    ...
    if args.load_pretrain_extr_model != None:
        extr = torch.load(args.load_pretrain_extr_model,
                          map_location="cpu")
    else:
        extr = model.BertExtractor(model_path=args.model_path,
                                        max_length=args.max_length)
    ...
    if args.load_pretrain_user_model != None:
        ...
        user_extr = model.MLP(model.FEATURE_NUM,
                              model.FEATURE_NUM,
                              args.emb_dim)
    ...
    if args.load_pretrain_course_model != None:
        ...
        course_extr = model.MLP(model.FEATURE_NUM,
                                model.FEATURE_NUM,
                                args.emb_dim)
                ...
                if (u_step + 1) % args.steps == 0 or\
                    ...
                    if args.do_log:    
                        with open(args.log_file, "r") as f:
                            log = json.load(f)
                    if validset != None:
                        uids, predicts, valid_loss, valid_mAP = predict_step(
                    ...
                    if args.do_log:
                        with open(args.log_file, "w") as f:
                            json.dump(log, f,
                                      ...
                                      ensure_ascii=False)
    if testset != None:
        if args.load_extr_model_test != None:
            extr = torch.load(args.load_extr_model_test,
                              map_location=args.device)
        else:
            extr = torch.load(args.save_extr_model_file,
                              map_location=args.device)
        extr.eval()
